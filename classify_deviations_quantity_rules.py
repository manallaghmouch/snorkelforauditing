...
@labeling_function()
def tp1l(x):
    return TP if not "Create PR,Approve PR" in x.tracestring and not "Approve PR,Create PO" in x.tracestring \
            else ABSTAIN


@labeling_function()
def tp2l(x):
    return TP if not re.search("^Create PR", x.tracestring, flags=re.I) and not re.search("Approve PR{1}", x.tracestring, flags=re.I)\
        else ABSTAIN


@labeling_function()
def tp3l(x):
    return FP if not "Create PR,Approve PR" in x.tracestring and not "Approve PR,Create PO" in x.tracestring \
            else ABSTAIN

tp_l = [tp1l,tp2l,tp3l,tp6l,tp7l,tp8l,tp9l,tp10l,tp11l,tp12l,tp13l,tp14l]
...
for j in range(0,11):
    for i in range(1,16):
        ...
        print(j)
        print(i)
        # Following line differs per setup (now initialized on zero)
        lfs = random.sample(tp_h, k=0) + random.sample(tp_m, k=0) + random.sample(tp_l, k=0) + \
              random.sample(fp_h, k=0) + random.sample(fp_m, k=0) + random.sample(fp_l, k=0)
        print(lfs)
        ...
        df_valid["lm_label"]=label_model.predict(L=L_valid,tie_break_policy="random")
        df_valid["mv_label"]= majority_model.predict(L=L_valid,tie_break_policy="random")
        df_valid["label_true"]=Y_valid

        subset_lm = df_valid[df_valid["label_true"] == df_valid["lm_label"]]
        subset_lm1 = subset_lm[subset_lm["label_true"] == 1]
        anomaly_correct_count_lm = subset_lm1["lm_label"].count()/452
        anom_correct_lm.append(anomaly_correct_count_lm)

        subset_mv = df_valid[df_valid["label_true"] == df_valid["mv_label"]]
        subset_mv1 = subset_mv[subset_mv["label_true"] == 1]
        anomaly_correct_count_mv = subset_mv1["mv_label"].count()/452
        anom_correct_mv.append(anomaly_correct_count_mv)
    ...
    mean_anom_mv.append(statistics.mean(anom_correct_mv))
    mean_anom_lm.append(statistics.mean(anom_correct_lm))
